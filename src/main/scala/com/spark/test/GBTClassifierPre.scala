...
object GBTClassifierPre {
  def main(args: Array[String]) {
    Logger.getLogger("org.apache.spark").setLevel(Level.WARN)
    System.setProperty("spark.sql.warehouse.dir","file:///F:/XWork/IDEAwork/recomwork/spark-warehouse")
    val sparkConf = new SparkConf().setAppName("GBTClaPre").setMaster("local")//.set("spark.kryoserializer.buffer.max","64")
    ...
    val output =new File("GBTresult1.csv");
