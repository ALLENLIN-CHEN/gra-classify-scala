...
object GBTClassifierPreNew {
  def main(args: Array[String]) {
    Logger.getLogger("org.apache.spark").setLevel(Level.WARN)
    System.setProperty("spark.sql.warehouse.dir","file:///F:/XWork/IDEAwork/recomwork/spark-warehouse")
    val sparkConf = new SparkConf().setAppName("GBTClaPreNew").setMaster("local")//.set("spark.kryoserializer.buffer.max","64")
    ...
    val rawdata = sqlContext.read.format("com.databricks.spark.csv").option("header","true").schema(customSchema).load("E:\\traindata\\ga-gbt\\train\\train20.csv").withColumnRenamed("action_type", "label").drop("data_time").na.drop
    rawdata.printSchema()
    ...
    CFResult_DF.show()*/
    val cf_schema = StructType(Array(
      StructField("user_id", DoubleType, true),
      StructField("job_id", DoubleType, true),
      StructField("rating", DoubleType, true)
    ))
    val CFResult_DF = sqlContext.read.format("com.databricks.spark.csv").option("header","true").schema(cf_schema).load("E:\\traindata\\ga-gbt\\CF_result_new50.csv").drop("date_time").na.drop
    CFResult_DF.printSchema()
    println(CFResult_DF.count())
    ...
    val testSchema = StructType(Array(
      ...
      StructField("job_worktime", DoubleType, true),
      StructField("label", DoubleType, true)))
    val test_data = sqlContext.read.format("com.databricks.spark.csv").option("header","true").schema(testSchema).load("E:\\traindata\\ga-gbt\\testdata_new.csv").drop("label").drop("data_time").na.drop
    ...
    val output =new File("E:\\traindata\\ga-gbt\\result\\GBTresult50_20.csv")
    val writer = new PrintWriter(output)
    val result_DF2 = result_DF.select("userid", "jobid", "rating", "prediction")
    val result_DF2Rows = result_DF2.collect()
    result_DF2Rows.foreach( row => {
      val tem = "%.2f".format(row.get(2))
      writer.write(row.get(0) + "," + row.get(1) + "," + tem + "," + row.get(3) +"\n")
    })
    sc.stop()
  }
